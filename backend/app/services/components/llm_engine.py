...
class LLMEngineComponent(Component):
    ...
    async def run(self, payload: Dict, context: ComponentContext) -> ComponentResult:
        ...
        # Get LLM settings from payload (passed from workflow nodes)
        provider = payload.get("provider", "google")
        model = payload.get("model")
        system_prompt = payload.get("systemPrompt") or payload.get("prompt")
        temperature = payload.get("temperature", 0.7)
        max_tokens = payload.get("maxTokens", payload.get("max_tokens", 2048))
        ...
        try:
            ...
            logger.error("llm_engine_failed", error=str(e), provider=provider, model=model)
